...
class Apartment(GeometricFigure):
    def __init__(self, points: List[Tuple[float, float]],
        ...
        # Генерируем планировку квартиры в зависимости от типа
    ...
    def _allocate_room_cells(self, remaining_cells, min_cells, max_cells):
        ...
        if self.starting_corner_cells is not None and len(self.starting_corner_cells) >= 1:
            start_cell = self.starting_corner_cells.pop()
        else:
            start_cell = random.choice(remaining_cells)
        queue = [start_cell]

        while queue and len(room_cells) < (room_cell_count+40):
    ...
    def _update_cell_properties(self, room_cells):
        ...
        # total_cells_needed = random.randint(min_cells, max_cells)
        #
        # for _ in range(total_cells_needed):
        #     if not remaining_cells:
        #         break  # Если больше нет доступных ячеек, выходим
        #
        #     # Выбираем первую доступную ячейку
        #     current_cell = remaining_cells.pop(0)
        #     room_cells.append(current_cell)
        #
        # return room_cells
    ...
    def _get_rooms_cell_range(self, room_table, cell_size):
        ...
        min_cells = int((self.area / total_rooms) / cell_area)
        max_cells = int((self.area / total_rooms) / cell_area)
